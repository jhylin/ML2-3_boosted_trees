...
The [definition of "NaN" assigned to max_phase](https://chembl.gitbook.io/chembl-interface-documentation/frequently-asked-questions/drug-and-compound-questions#what-is-max-phase) indicates that compounds labelled as "NaN" or "null" have no evidence of showing they've reached clinical trials, so I'm removing them from the dataset(this can also be included in the dataset depending on project goals). 

A max_phase of -1 is assigned to molecules with unknown clinical phase status ([ChEMBL reference](https://chembl.gitbook.io/chembl-interface-documentation/frequently-asked-questions/drug-and-compound-questions#what-is-max-phase)), so I'm also going to drop them for this particular experiment.
...
This post is going to focus only on Scikit_mol which has a different way to handle SMILES errors. Another useful way to deal with SMILES errors is [Molpipeline](https://github.com/basf/MolPipeline)'s SMILES error handling, with an example shown in one of [its notebooks](https://github.com/basf/MolPipeline/blob/main/notebooks/03_error_handling.ipynb). The main difference from what I could see, even though I haven't got to use molpipeline yet, was that molpipeline takes into account of all the invalid SMILES by giving each invalid SMILES a "NaN" label in the pipeline process - this maintains the matrix shape and good for tracking down the problematic SMILES (molecules).
...
Also, manually checking for any "NaNs" in the canonical SMILES column (X variable), since AdaBoostClassifier won't accept missing values in the dataset, but if using HistGradientBoostingClassifier() instead, it should take care of the native NaNs.

```{python}
print(f"{df_ache.canonical_smiles.isna().sum()} out of {len(df_ache)} SMILES failed in conversion")
```

There are other ways to deal with NaNs with a few examples provided by [scikit-learn](https://scikit-learn.org/stable/modules/impute.html). However, with regards to drug discovery data, there might be more caveats that need to be taken during data preprocessing (I'm also still exploring this too). 
...
This is also the time to think about generating features for model training. For RDKit 2D descriptors, scikit_mol has a nice way of quickly identifying what are available based on RDKit's [rdkit.Chem.Descriptors module](https://www.rdkit.org/docs/source/rdkit.Chem.Descriptors.html#module-rdkit.Chem.Descriptors) and [rdkit.ML.Descriptors.MoleculeDescriptors module](https://www.rdkit.org/docs/source/rdkit.ML.Descriptors.MoleculeDescriptors.html#rdkit.ML.Descriptors.MoleculeDescriptors.MolecularDescriptorCalculator).
...
Note: can't use all 209 RDKit2D descriptors for AdaBoostClassifier() as some of the descriptors will be "0" and AdaBoostClassifier() won't take care of NaNs, so using a selection of descriptors only (but HistGradientBoostingClassifier() should be able to take into account of NaNs).

Here's an example of building a scikit_learn pipeline by using AdaBoostClassifier model, along with scikit_mol's transformers, for multi-class max_phase predictions (training set consists of max_phase 1, 2, 3 and 4).
...
print(f"Testing score for mlpipe_adaboost: {mlpipe_adaboost.score(X_test, y_test):0.2f}")
print("AdaBoostClassifier's misclassification_error: "f"{mlpipe_adaboost_misclassification_error:0.3f}")

print(f"Training score for mlpipe_adaboost: {mlpipe_xgb.score(X_train, y_train):0.2f}")
print(f"Testing score for mlpipe_adaboost: {mlpipe_xgb.score(X_test, y_test):0.2f}")
print("XGBClassifier's missclassification_error: "f"{mlpipe_xgb_misclassifiaction_error:0.3f}")
```

It appears that XGBoost model obtained a better prediction accuracy than AdaBoost one (although the models were built in a very simple and crude way, but shows the slight difference in performance). The training data being used here is also very imbalanced with a lot of them being max_phase of "0" than "4", and with max_phase "4" being our ultimate aim, the dataset used above is really for demonstration only. Also, since this post is already quite long, I'd rather not make this post into a gigantically tl;dr, so for the imbalanced data discussion and exploration, please refer to my previous posts on [random forest](**add link**) and [random forest classifier](**add link**).
...
A full list of XGBoost classifier pipeline parameters and settings used can be retrieved as shown below. It contains a *long* list of parameters and one of the ways to find the most optimal set of parameters is by using *cross-validation* (CV).
...
To obtain only the parameter names for ease of tuning.

```{python}
# Get parameter names only to tune
mlpipe_xgb.get_params().keys()
```

Some of the main XGBoost parameters that can be tuned are n_estimators, max_depth, learning_rate, subsample and reg_lambda. Here, I'm going to try to look for the best combination of learning_rate and subsample for a XGBoost classifier model.

```{python}
# Parameters and distributions to be sampled
params_dist = {
    # learning_rate usually between 0.01 - 0.1 as suggested by Raschka et al. Default is between 0 and 1
...
print(f"The best cv score found from grid search is: {grid_search.best_score_:0.2f}")
print(f"The best cv parameter settings found from grid search are: {grid_search.best_params_:0.2f}")

# Run time on 3/6/24 total time taken: 13m 29.4s
# Fitting 5 folds for each of 9 candidates, totalling 45 fits
#0.9885159010600708
# {'xgbclassifier__learning_rate': 0.3, 'xgbclassifier__subsample': 0.7}
```

For tuning parameters of Morgan fingerprints, this scikit_mol [example notebook](https://github.com/EBjerrum/scikit-mol/blob/main/notebooks/06_hyperparameter_tuning.ipynb) explains how to do it with code.

```{python}
# ?try to change parameter settings for learning_rate & subsample by using .set_params()
```
...
## must open in binary format to pickle ("pickle save")
# pickle.dump(clf, open("best_calif.pkl", "wb"))
## ("pickle run")
# clf2 = pickle.load(open("best_calif.pkl", "rb"))
## Check the pickled model is the same as the original model 
## ref. for np.allclose - https://numpy.org/doc/stable/reference/generated/numpy.allclose.html#numpy-allclose
# print(np.allclose(clf.predict(X), clf2.predict(X)))


# pickle example 3---
## alternatively, you can pickle the booster ("pickle save")
# pks = pickle.dumps(bst2)
## load model and data in ("pickle run")
# bst3 = pickle.loads(pks)
# preds3 = bst3.predict(dtest2)
## To check if the pickled model was same as the original model
