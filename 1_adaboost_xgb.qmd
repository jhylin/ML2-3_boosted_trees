...
Please note: the only molecule with max_phase of "0.5" was converted into "0" after I've converted the datatype of max_phase from float to int (I've left it deliberately like this since this is only a sample demonstration on using scikit_learn's pipeline along with scikit_mol, but in reality this should be handled with care i.e. don't change it as different max phase values have different meanings!). Therefore the following value count on max_phase will have 6411 molecules with max_phase "0", rather than the previous number of 6410.
...
Binary classification has been used in my previous posts, e.g. target as max_phase 4 (re-labelled as "1") with training set of max_phase "null" re-labelled as "0". This time I'll be using multi-class classification to predict max_phase of the training set of molecules containing 0, 1, 2, 3, 4.
