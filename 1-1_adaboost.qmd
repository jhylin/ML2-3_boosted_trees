---
title: "Boosted trees"
subtitle: "Series 2.3.1 - AdaBoost, Gradient boosting and Scikit-mol"
...
- A sample demo using scikit_learn's 
- AdaBoostClassifier, xgboost & scikit_mol's transformers
...
table = [
    ["trains weak learners that are usually deeper with a max depth of 3 to 6 (or max number of 8 to 64 leaf nodes)","trains weak learners (decision tree stumps) based on errors from previous decision tree stump"], 
    ["uses prediction errors directly to produce the target variable to fit the next tree","uses prediction errors to assign sample weights"], 
    #["", ""]
    ]
Markdown(tabulate(
  table, 
  headers=["XGBoost","AdaBoost"]
))
```
...
This is only a quick boosted tree demonstration  and a very simple example that is very likely not going to reflect real-life scenarios, so please refer to this with some cautions.
