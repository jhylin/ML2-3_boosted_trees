---
title: "Boosted trees"
subtitle: "Series 2.3.1 - Adaptive boost, gradient boosting and scikit-mol"
...
- A sample demo using scikit_learn's AdaBoostClassifier, xgboost & scikit_mol's transformers
...
Essentially a similar concept is behind the gradient boosted trees where a series of weak learners is trained in order to create a stronger ensemble of models [@mlbook2022]. However, some differences between these two types of boosted trees.

```{python}
#| echo: false
#| tbl-cap: Differences between XGBoost and AdaBoost
...
In scikit-learn there are two types of gradient boosted trees. One way to choose when to use which one seems to be based on the sample size at first. [GradientBoostingClassifer()](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.GradientBoostingClassifier.html#sklearn.ensemble.GradientBoostingClassifier) class is likely better when there is only a small sample size (e.g. when number of sample is less than 10,000), while [HistGradientBoostingClassifier()](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.HistGradientBoostingClassifier.html#sklearn.ensemble.HistGradientBoostingClassifier) class is likely better when your sample size is at least 10,000+ or more. The HistGradientBoostingClassifier() is a histogram-based gradient boosting classification tree that is mainly inpired by [LightGBM](https://github.com/Microsoft/LightGBM). 

XGBoost or extreme gradient boosting is another commonly used open-source package, originally developed at the University of Washington by T. Chen and C. Guestrin, that uses stochastic gradient boosting to build an ensemble of predictive models.
...
This is only a quick boosted tree demonstration  and a very simple example that is very likely not going to reflect real-life scenarios (use with caution). 
...
#     Standardizer(),
#     # RDKit2D descriptors (or MorganFingerprintTransformer())
