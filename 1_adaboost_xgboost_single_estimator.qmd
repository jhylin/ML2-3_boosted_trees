...
```{python}
# Convert canonical_smiles into rdkit_mol
...
df_merge = pd.merge(df_prep, df_2d, left_index=True, right_index=True)
```
...
```{python}
df_merge.dropna(axis="index", how="any", inplace=True)
```
...
```{python}
X = df_merge.iloc[:, 14:19]
y = df_merge.max_phase
```
...
---Use individual estimator (rather than pipeline)---
...
**Confirmed single estimator was able to be fitted with training data and also able to predict on X_test**
...
#mlpipe = make_pipeline(
    ...
    # MolecularDescriptorTransformer(desc_list=['qed']),
...
**X (n_sample, n_features) - missing n_features but should be generated in MolecularDescriptorTransformer()?**

**Problem with "Found input variables with inconsistent numbers of samples: [531, 133]"**
