...
```{python}
# Try binary class of max_phase first
```
...
print(f"{df_prep.standard_smiles.isna().sum()} out of {len(df_ache)} SMILES failed in conversion")
```
...
X_train, y_train, X_test, y_test = train_test_split(df_prep.rdkit_mol, df_prep.max_phase, test_size=0.2, random_state=3)
```
