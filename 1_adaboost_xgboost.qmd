...
table = [
    ["trains weak learners (decision tree stumps) based on errors from previous decision tree stump", 
    ...
    "uses a global learning rate (learning_rate) for each tree"]
    ]

Markdown(tabulate(table, headers=["AdaBoost", "XGBoost"]))
```

XGBoost or extreme gradient boosting [@DBLP:journals/corr/ChenG16] is one of the most commonly used open-source package, originally developed at the University of Washington by T. Chen and C. Guestrin, that uses stochastic gradient boosting to build an ensemble of predictive models. [XGBoost documentation](https://xgboost.readthedocs.io/en/stable/index.html) - https://xgboost.readthedocs.io/en/stable/index.html

Main parameters to tune [@bruce2020]: 

- subsample - controls fraction of observations that should be sampled at each iteration or subsample ratio of the training instance (as quoted from XGBoost's Python API reference via its [Scikit-learn API](https://xgboost.readthedocs.io/en/latest/python/python_api.html#module-xgboost.sklearn))

- eta or learning-rate (it's currently named as "learning_rate" in scikit-learn and XGBoost) - shrinkage factor applied to alpha in the boosting algorithm or it may be more easily understood as the boosting learning rate used to prevent overfitting

In scikit-learn, there are two types of gradient boosted tree methods, GradientBoostingClassifer() and HistGradientBoostingClassifier() (note: regressors are also available), in its sklearn.ensemble module. One way to choose between them is to check sample size first. [GradientBoostingClassifer()](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.GradientBoostingClassifier.html#sklearn.ensemble.GradientBoostingClassifier) class is likely better when there is only a small sample size (e.g. when number of sample is less than 10,000), while [HistGradientBoostingClassifier()](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.HistGradientBoostingClassifier.html#sklearn.ensemble.HistGradientBoostingClassifier) class is likely better when your sample size is at least 10,000+ or more. The HistGradientBoostingClassifier() is a histogram-based gradient boosting classification tree that is mainly inpired by [LightGBM](https://github.com/Microsoft/LightGBM). 

Rather than making the introduction part too long in this post, please refer to this book by [@mlbook2022] for further details on tree models.
...
*Dealing with max_phase labelled "nan"*

The [definition of "nan" assigned to max_phase](https://chembl.gitbook.io/chembl-interface-documentation/frequently-asked-questions/drug-and-compound-questions#what-is-max-phase) indicated that compounds labelled as "nan" or "null" had no evidence showing they've reached clinical trials, so it would probably (depend on project goals) be best to remove them from the dataset.

Removing all "nan" compounds in max_phase column.
...
Ideally, AdaBoost is not as useful as XGBoost (explained in [@mlbook2022]). So the focus on hyperparameter tuning will be on XGBoost. However, it contains a long list of parameters and one of the ways to find the most optimal set of parameters is via using cross-validation.
...
Try using pickle to download model and parameters